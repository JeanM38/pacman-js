...
export class Ghost extends Character {
  constructor (originalX, originalY, wallsInfo, pacman) {
    ...
    eyes.style.width = `${ct.widthGhost}px`;
    ...
    tail.style.background = `linear-gradient(-45deg, transparent 75%, ${ct.colorsGhost[i]} 75%) 0 50%, linear-gradient( 45deg, transparent 75%, ${ct.colorsGhost[i]} 75%) 0 50%`;
    tail.style.backgroundSize = "10px 10px, 10px 10px";
    this.ghost = document.createElement('div');
    this.ghost.setAttribute('class', 'ghost');
    this.ghost.style.height = `${ct.heightGhost}px`;
    this.ghost.style.width = `${ct.widthGhost}px`;
    this.ghost.style.borderRadius = '30% 30% 0 0'
    this.ghost.style.backgroundColor = ct.colorsGhost[i];
  ...
  initialization = (_self) => {
    _self.intervalId = setInterval(() => {
      function getRandomMove() {
        ...
        let randomMove = ct.directions[randomIndex];
        if(randomMove === 'ArrowUp' || randomMove === 'ArrowDown') {
          if(_self.countTop < ct.ghostLimitStraightLine && !_self.switchTop) {
            randomMove = _self.biasDirectionY;
            _self.countTop += 1;
          } else if (_self.countTop == ct.ghostLimitStraightLine && !_self.switchTop) {
            _self.countTop = 0;
        ...
        if(randomMove === 'ArrowLeft' || randomMove === 'ArrowRight') {
          if(_self.countRight < ct.ghostLimitStraightLine && !_self.switchLeft) {
            randomMove = _self.biasDirectionX;
            _self.countRight += 1;
          } else if (_self.countRight == ct.ghostLimitStraightLine && !_self.switchLeft) {
            _self.countRight = 0;
          ...
          _self._super.handleMove(randomMove, ct.stepGhost);
  ...
  updatePosition = () => {
    ...
    if (Math.abs(this.posY - this.pac.getPosY() - ct.sizePacman) < 15 ||
        Math.abs(this.pac.getPosY() - this.posY - ct.sizePacman)  < 15 
    ) {
    ...
    if( Math.abs(this.pac.getPosX() + ct.sizePacman - this.posX) < 10 ||
        Math.abs(this.posX + ct.widthGhost - this.pac.getPosX()) < 10
    ) {
